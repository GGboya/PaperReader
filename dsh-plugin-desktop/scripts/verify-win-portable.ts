...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `PaperReader-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'PaperReader.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing PaperReader.exe: ${portablePath}`)
  }
