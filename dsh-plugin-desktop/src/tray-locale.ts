...
const labels: Record<DesktopLocale, Record<DesktopTrayLabelKey, (value: string) => string>> = {
  en: {
    ...
    downloadingUpdate: version => `Downloading PaperReader ${version}…`,
    ...
    updateAvailable: version => `PaperReader ${version} Available`,
  },
  zh: {
    addProfile: () => '新建 Profile…',
    checkForUpdates: () => '检查更新…',
    checkingForUpdates: () => '正在检查更新…',
    downloadingUpdate: version => `正在下载 PaperReader ${version}…`,
    ...
    updateAvailable: version => `PaperReader ${version} 可用`,
  },
}
...
export const rendererRecoveryCopy: Record<DesktopLocale, DesktopRestartConfirmationCopy> = {
  en: {
    title: 'Restore PaperReader',
    message: 'The interface could not recover automatically.',
    detail: 'Automatic recovery stopped after repeated failures to avoid a restart loop. You can try again without restarting the background service. Unsent input may be lost. Export diagnostics from the tray to investigate. Choose Open PaperReader from the tray to return to this prompt later.',
    confirm: 'Try recovery again',
    cancel: 'Not now',
  },
  zh: {
    title: '恢复 PaperReader',
    message: '界面未能自动恢复。',
    detail: '自动恢复连续失败，为避免重启循环已暂停。可以再次尝试恢复，无需重启后台服务。未发送的输入可能丢失。请从托盘导出诊断信息以继续调查。稍后可从托盘选择“打开 PaperReader”再次打开此提示。',
    confirm: '再次尝试恢复',
    cancel: '暂不处理',
  },
}
...
const restartConfirmationCopy: Record<DesktopLocale, Record<'normal' | 'recovery', DesktopRestartConfirmationCopy>> = {
  en: {
    normal: {
      title: 'Restart PaperReader',
      message: 'Restart PaperReader now?',
    ...
    recovery: {
      ...
      message: 'Restart PaperReader in Recovery Mode?',
  ...
  zh: {
    normal: {
      title: '重启 PaperReader',
      message: '现在重启 PaperReader？',
    ...
    recovery: {
      title: '重启到恢复模式',
      message: '重启 PaperReader 并进入恢复模式？',
