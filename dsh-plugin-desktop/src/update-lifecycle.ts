...
function updateAvailableNotification(locale: DesktopLocale, version: string): DesktopNotification {
  return locale === 'zh'
    ? { title: 'PaperReader 有可用更新', body: `版本 ${version} 已可下载。打开 PaperReader 即可继续。` }
    : { title: 'PaperReader Update Available', body: `Version ${version} is ready to download. Open PaperReader to continue.` }
}
