/** Headless-safe npm launcher for the PaperReader Electron executable. */
...
Launch PaperReader with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate PaperReader diagnostics')
    }
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged PaperReader application.\n',
    )
    return 1
  }
